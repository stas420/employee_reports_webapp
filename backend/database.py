...
if __name__ == '__main__':
    ...
    salt = bc.gensalt()
    pw_hash = bc.hashpw('password'.encode(), salt)
    print(pw_hash.decode())
    user = User('example_user_2137', pw_hash.decode(), 'path/to/photo', True)
    #user.insert_into_db(connection)
    #connection.commit()
    print(bc.checkpw('password'.encode(), user.hash_password.encode()))
    #connection.close()