...
class Timestamp:
    def __init__(self, id : UUID, employee_id : str, start_time : datetime, end_time : datetime, start_photo_path : str, end_photo_path : str):
        self.id = id
    ...
    def insert_into_db(self, connection: Connection) -> bool:
        ...
        cursor = connection.cursor()
        cursor.execute('INSERT INTO timestamps (entry_id, employee_id, start_time, end_time, start_photo_path, end_photo_path) VALUES (?, ?, ?, ?, ?, ?)', (str(self.id), self.employee_id, self.start_time.isoformat(), self.end_time.isoformat(), self.start_photo_path, self.end_photo_path))
        return True
    
    def update_in_db(self, connection: Connection) -> bool:
        if db.fetch_timestamp(connection, self.id) is None:
            return False

        cursor = connection.cursor()
        cursor.execute('UPDATE timestamps SET employee_id = ?, start_time = ?, end_time = ?, start_photo_path = ?, end_photo_path = ? WHERE entry_id = ?', (self.employee_id, self.start_time.isoformat(), self.end_time.isoformat(), self.start_photo_path, self.end_photo_path, str(self.id)))
        return True
